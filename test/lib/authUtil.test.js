...
describe('test authUtil model', () => {
    ...
    describe('test getJwt function', () => {
        ...
        it('should request a new JWT if none exists', async () => {
            ...
            const mockDecodedJwt = { zid: 'test-tenant', scope: ['scope1'], ext_attr: { zdn: 'test-subdomain' } };
            const mockReq = { error: jest.fn() };
            const mockSvc = { uaa: {} };
            const mockAuthInfo = { getSubdomain: jest.fn().mockReturnValue('test-subdomain') };
            ...
            cds.context = { http: { req: { authInfo: mockAuthInfo } } };
            xssec.requests.requestClientCredentialsToken.mockImplementation((_, __, ___, ____, callback) => {
        ...
        it('should request a new JWT if existing one is expired', async () => {
            ...
            const mockDecodedJwt = { zid: 'test-tenant', scope: ['scope1'], ext_attr: { zdn: 'test-subdomain' }, exp: Math.floor(Date.now() / 1000) - 3600000 };
            const mockReq = { error: jest.fn() };
            const mockSvc = { uaa: {} };
            const mockAuthInfo = { getSubdomain: jest.fn().mockReturnValue('test-subdomain') };
