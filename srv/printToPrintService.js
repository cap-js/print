...
const { getDestinationFromServiceBinding, transformServiceBindingToClientCredentialsDestination, } = require("@sap-cloud-sdk/connectivity")
  ...
  const destination = await getDestinationFromServiceBinding(
    {
      ...
      serviceBindingTransformFn: async function (service, options) {
        return transformServiceBindingToClientCredentialsDestination(
          Object.assign(service, { credentials: service.credentials.uaa, url: "https://api.eu12.print.services.sap" }), // TODO: make dynamic 
          options
        )
      }
    }
