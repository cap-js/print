  ...
  for (let [entityName, entity] of Object.entries(m.definitions)) {
    ...
    if (numberOfFiles > 1)
      m.definitions[`${serviceName}.PrintServiceFiles`] = {
        [FILE_ENTITY]: true,
        "@Common.Label": "{i18n>PRINT_FILES}",
        kind: "entity",
        projection: {
          from: { ref: ["PrintService.Files"] },
        },
        elements: {
          entity: { key: true, type: "cds.String", "@Common.Label": "{i18n>ENTITY}" },
          fileName: {
            type: "cds.String",
            "@Common.Label": "{i18n>FILE_NAME}",
          },
          property: {
            key: true,
            type: "cds.String",
            ...
            "@Common.Label": "{i18n>PROPERTY}",
          },
          ...Object.entries(entity.elements)
            .filter(([, def]) => def.key)
            .map(([,], idx) => [
              `entityKey${idx + 1}`,
              { type: "cds.String", "@Common.Label": "{i18n>ENTITY_KEY}" },
            ])
            .reduce((acc, [name, def]) => {
              acc[name] = def;
              return acc;
            }, {}),
          label: { type: "cds.String", "@Common.Label": "{i18n>LABEL}" },
        },
      };
  }
